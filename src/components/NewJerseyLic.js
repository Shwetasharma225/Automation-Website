...
const NewJerseyLic = ({ userId, onDataCheck }) => {
 ...
 if (responseData.Status === "204" && responseData.Msg === "Record Not Found!!!") {
  ...
  //setShowNoLicenseMessage(true);
        ...
        if (JSON.stringify(responseData) === '{"Status":"204","Msg":"Record Not Found!!!","Data":null}') {
          onDataCheck(true); // Pass the result back to the parent component
        }
        setIsLoading(false);
        setError(null);
      } catch (error) {
                ...
                className="apex-chart"
                  options={{
                    ...
                    legend: {
                      position: 'top',
                      onItemClick: {
                        toggleDataSeries: true, // Prevent hiding series on legend click
                      },
                      onItemHover: {
                        highlightDataSeries: true, // Prevent highlighting series on legend hover
                      },
                    },
                    toolbar: {
                      show: false,
